...
GitHub-authenticated Lean upload
  ...
  → atomic encrypted source archive and FIFO admission
...
Formal uploads enter a durable FIFO and exactly one proof verifier runs at a time. The rate limit is **three admitted uploads per authenticated GitHub account per UTC calendar day**, resetting at **00:00 UTC**. Input rejected before durable queue admission does not use a slot and is not archived. Once admitted, every terminal outcome uses that slot—including Lean syntax or elaboration errors, statement or axiom mismatch, kernel rejection, timeout, and sandbox expiration. Admitted slots are not automatically refunded. Waiting proofs are sealed in paused, no-egress E2B sandboxes; advancing the queue resumes only its head.

Every admitted server-generated manifest and exact `Solution.lean` is gzip-compressed and encrypted with AES-256-GCM before it is committed to the automation branch. The encrypted envelope and queue transition share one non-forced compare-and-swap commit, so a queue entry cannot be admitted without its durable source archive. The public branch contains ciphertext, HMAC-pseudonymous ownership counters, opaque job coordinates, digests, and terminal receipts—never GitHub usernames or rejected Lean source in plaintext. Only configured maintainers can decrypt an archive through `/admin/submissions`; accepted proofs are separately published as permanent Apache-2.0 evidence. Archives are retained indefinitely while the service operates. `SUBMISSION_ARCHIVE_KEY` must be backed up because losing or rotating it makes earlier archives unreadable.

The durable queue is also the finalization authority: a signed browser handle or E2B lifecycle event cannot publish a sandbox that was never admitted. Launch transitions are idempotent under a per-job `flock`; if verification finished but result sealing was interrupted, the queue runner replays only the sealed finalizer before considering a full verifier restart. Competitive starts defer while the operator-only production flow test is active, and the flow-test route stops its sandbox if a competitive entry wins the admission race.
...
Production deploys run from `.github/workflows/deploy-production.yml` on every push to `main`. Because the promotion credential is not GitHub Actions' `GITHUB_TOKEN`, a successful record update triggers the normal deployment. An authenticated status request publishes immediately; if the submitter closes the browser, a signed E2B pause webhook resumes the preserved sandbox, completes the same idempotent publication path, and advances the next FIFO entry. `.github/workflows/submission-queue-sweep.yml` runs the same idempotent reconciliation every 15 minutes with the shared `CRON_SECRET`; the daily Vercel cron remains an independent final backstop because Hobby projects cannot schedule more frequently there.
