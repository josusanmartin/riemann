...
import { queuedVerificationRunnerCommand } from "@/lib/e2b-queue";
...
describe("queued E2B runner recovery", () => {
  it("uses one lock and finalizes an existing proof artifact before rerunning", () => {
    ...
    expect(command).toContain(`if [[ -s /var/lib/riemann/jobs/${jobId}/result.json ]]`);
    expect(command).toContain(`if [[ -s /home/riemann/jobs/${jobId}/output/attestation.json ]]`);
  ...
  it("rejects job coordinates before constructing a shell command", () => {
    expect(() =>
      queuedVerificationRunnerCommand(`${jobId}; touch /tmp/injected`, proofDigest),
    ).toThrow();
    expect(() =>
